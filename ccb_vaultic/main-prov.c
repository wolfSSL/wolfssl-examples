/* wolfCrypt prov application for Wisekey VaultIC
 *
...
int main(int argc, char **argv)
{
    ...
    if (rc == 0) {
        ...
        ctx_storage.config = &gManuConfig;
        ...
        if (rc == 0) {
            ...
            if (rc == 0) {
                /* Perform Load action */
                rc = ccbVaultIc_ProvisionAction(ctx, &gProvision);

                if (rc == 0) {
                    printf("Provision succeeded.\n");
                }
                else
                {
                    printf("Provision failed with %d vlt_rc:%x\n", rc, ctx_storage.vlt_rc);
                }
