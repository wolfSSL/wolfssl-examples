                       ...
                       int keySz, char* name)
{
    ...
    if (name != NULL) {
        ...
        wolfSSL_EC_KEY_set_group(key, group);
    }
    ...
    switch(directive) {
        ...
        case ECPARAM:
            {
                if (fmt == PEM_FORM) {
                    wolfSSL_PEM_write_bio_ECPrivateKey(bioOut, key, NULL, NULL, 0,
                            NULL, NULL);
                }
                else {
        ...
        case PUB_ONLY:
            if (fmt == PEM_FORM) {
                wolfSSL_PEM_write_bio_EC_PUBKEY(bioOut, key);
            }
            else {
                ...
                der = XMALLOC(derSz, HEAP_HINT, DYNAMIC_TYPE_TMP_BUFFER);
