...
static int do_certgen(int argc, char** argv)
{
    ...
    char sapkiFile[] = "../certs/dilithium_level5_ca_pubkey.der";
    char altPrivFile[] = "../certs/dilithium_level5_ca_key.der";
#else
    char caCert[] = "./ca-cert-pq.der";
    char newCertOutput[] = "./server-cert-pq.der";
    char sapkiFile[] = "../certs/dilithium_level5_server_pubkey.der";
    char altPrivFile[] = "../certs/dilithium_level5_server_key.der";
    ...
    ret = wc_dilithium_set_level(&altCaKey, 5);
    if (ret < 0) goto exit;
    ...
    altSigAlgSz = SetAlgoID(CTC_DILITHIUM_LEVEL5, altSigAlgBuf, oidSigType, 0);
    ...
    ret = wc_MakeSigWithBitStr(altSigValBuf, altSigValSz, CTC_DILITHIUM_LEVEL5,
                               preTbsBuf, preTbsSz, DILITHIUM_LEVEL5_TYPE,
                               &altCaKey, &rng);
    if (ret < 0) goto exit;
    altSigValSz = ret;
