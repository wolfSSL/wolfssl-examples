...
int wc_InitRsaHw(RsaKey* key)
{
    ...
    /* always load public parts of key */
    ...
    if (mp_to_unsigned_bin(&(key->e), (byte*)&e + (sizeof(word32) - eSz))
                != MP_OKAY) {
        XFREE(m, key->heap, DYNAMIC_TYPE_KEY);
    ...
    if (key->type == RSA_PRIVATE) {
        ...
        d = (unsigned char*)XMALLOC(dSz, key->heap, DYNAMIC_TYPE_KEY);
        if (d == NULL) {
            XFREE(m, key->heap, DYNAMIC_TYPE_KEY);
            return MEMORY_E;
        }
        if (mp_to_unsigned_bin(&(key->d), d)
                != MP_OKAY) {
            XFREE(m, key->heap, DYNAMIC_TYPE_KEY);
            XFREE(d, key->heap, DYNAMIC_TYPE_KEY);
            WOLFSSL_MSG("Unable to get RSA key exponent");
            return MP_READ_E;
        }
...
#ifdef WOLFSSL_XILINX_CRYPT
    XFREE(key->mod,    key->heap, DYNAMIC_TYPE_KEY);
    XFREE(key->priExp, key->heap, DYNAMIC_TYPE_KEY);
    key->priExp = NULL;
    key->mod    = NULL;
#endif
                          ...
                          word32* outLen, int type, RsaKey* key, WC_RNG* rng)
{
    ...
    case RSA_PRIVATE_ENCRYPT:
        if (XSecure_RsaPrivateDecrypt(&(key->xRsa), (byte*)in, inLen,
                                        out) != XST_SUCCESS) {
            ret = BAD_STATE_E;
        }
        break;
    ...
    case RSA_PUBLIC_DECRYPT:
        ...
        if (key->type == RSA_PRIVATE) {
            /* RSA private key type has public key parts */
            if (XSecure_RsaInitialize(&(key->xRsa), key->mod, NULL,
        ...
        if ((ret = XSecure_RsaPublicEncrypt(&(key->xRsa), (byte*)in, inLen,
                                        out)) != XST_SUCCESS) {
            ret = BAD_STATE_E;
        }
        if (key->type == RSA_PRIVATE) {
            /* re-initialize to state before encryption */
            if (XSecure_RsaInitialize(&(key->xRsa), key->mod, NULL,
                        key->priExp) != XST_SUCCESS) {
                WOLFSSL_MSG("Unable to initialize RSA on hardware");
                return BAD_STATE_E;
            }
        }
