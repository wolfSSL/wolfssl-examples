...
/* It is currently expected that the output buffer is
 * large enough to hold cipher text plus 16 byte tag. */
                                   ...
                                   const byte* authIn, word32 authInSz)
{
    ...
    /* API expects that output is size of input + 16 byte tag. */
    if (in != NULL) {
        XSecure_AesInitialize(&(aes->xilAes), &(aes->dma), aes->kup, (word32*)iv,
            aes->key_init);

        XSecure_AesEncryptData(&(aes->xilAes), out, in, sz);
                                   ...
                                   const byte* authIn, word32 authInSz)
{
    ...
    if (authIn != NULL && authInSz > 0) {
        byte tmp[AES_GCM_AUTH_SZ];

        /* generate tag for if input had no additional data */
        XMEMSET(initalCounter, 0, AES_BLOCK_SIZE);
        XMEMCPY(initalCounter, iv, ivSz);
        initalCounter[AES_BLOCK_SIZE - 1] = 1;
        tag = buf;
        ...
        /* compare generated tag of input to authTag passed in */
        XMEMCPY(tmp, tag, AES_GCM_AUTH_SZ);
        GHASH(aes, authIn, authInSz, in, sz, tmp, AES_GCM_AUTH_SZ);
        wc_AesEncryptDirect(aes, scratch, initalCounter);
        xorbuf(scratch, tmp, AES_GCM_AUTH_SZ);
        if (ConstantCompare(authTag, scratch, authTagSz) != 0) {
            ret = AES_GCM_AUTH_E;
        }
    }
    else {
    ...
    if (XSecure_AesDecryptData(&(aes->xilAes), out, (byte*)in, sz, tag)
                               != XST_SUCCESS) {
        ret = BAD_STATE_E;
    }
