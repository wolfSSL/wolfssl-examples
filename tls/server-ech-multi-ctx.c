 ...
 *   noack_return -> no SNI or unknown name: keep the default CTX, without
 *                   acking the SNI
 *   fatal_return -> matched tenant's CTX swap failed + aborts */
static int sni_select_tenant(WOLFSSL* ssl, int* ad, void* arg)
{
    ...
    /* no SNI provided, keep the default CTX in use */
