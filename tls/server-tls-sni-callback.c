...
/* Multi-SNI TLS server: a single WOLFSSL_CTX fronts multiple tenants based on
 * the SNI sent by the client. A servername callback dispatches on the SNI and
 * installs the matched tenant's certificate and private key on the
 * per-connection WOLFSSL object. Clients that send no SNI, or an SNI matching
 * no tenant, fall back to the default cert/key on the CTX.
 *
 ...
 *
 * returns:
 *   0            -> tenant match (cert/key installed)
 *   noack_return -> no SNI or unknown name: keep the default cert/key from
 *                   the CTX, without acking the SNI
...
static int sni_select_tenant(WOLFSSL* ssl, int* ad, void* arg)
{
    ...
    /* no SNI -> default cert and key already installed on the CTX */
    if (nameLen == 0 || name == NULL)
        return noack_return;

    fprintf(stdout, "Got client SNI: %.*s\n", (int)nameLen, (const char*)name);

    /* match one of the configured tenants and install its cert/key */
...
int main(void)
{
    ...
    while (!shutdown) {
        ...
        /* The client may abort after the handshake, so handle the
         * potential IO errors here */
