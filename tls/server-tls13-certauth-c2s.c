...
static int cert_setup_callback(WOLFSSL *ssl, void *_arg) {
    /* Retrieve the CA names sent by the client */
    WOLF_STACK_OF(WOLFSSL_X509_NAME)* names = wolfSSL_get0_peer_CA_list(ssl);

    char *certFileName = NULL;
    char *keyFileName = NULL;

    if (names) {
        /* Client sent CA names: read them and try to load a matching
         * certificate */
        int i;
        int num = wolfSSL_sk_X509_NAME_num(names);

        printf("CA certificate names received:\n");
        for (i = 0; i < num; i++) {
            WOLFSSL_X509_NAME* name = wolfSSL_sk_X509_NAME_value(names, i);
            char* buf = wolfSSL_X509_NAME_oneline(name, NULL, 0);
            printf("\t%s\n", buf);

            if (certFileName != NULL && keyFileName != NULL) {
                /* Certificate already found */
                continue;
            }

            /* Servers with multiple certificates issued by separate CAs might
             * want to check them all and load one issued by a CA that the
             * client supports. */
            if (strcmp(buf, "/C=US/ST=Montana/L=Bozeman/O=Sawtooth"
                        "/OU=Consulting/CN=www.wolfssl.com"
                        "/emailAddress=info@wolfssl.com") == 0) {
                certFileName = CERT_FILE;
        ...
        if (certFileName == NULL || keyFileName == NULL) {
            fprintf(stderr, "ERROR: CA names sent by client do not match any "
                    "available certificates\n");
            return 0;
        }
    }
    else {
        /* Client sent no CA names, fall back to default */
        certFileName = CERT_FILE;
        keyFileName = KEY_FILE;
    }
...
int main(int argc, char** argv)
{
    ...
    /* Do not load the server's key and certificate in advance. Instead set a
     * callback so that they can be selected on demand based on CA names
