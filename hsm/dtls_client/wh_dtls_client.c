 ...
 *      ./examples/server/server -u -v d \
 *          -c ./certs/server-ecc.pem -k ./certs/ecc-key.pem \
 *          -A ./certs/client-ecc-cert.pem -p 11111 -i
 *
...
static void Usage(const char* progName)
{
    ...
    printf("    ./examples/server/server -u -v d \\\n");
...
int main(int argc, char** argv)
{
    ...
    /*
     * The wolfHSM crypto callback, which intercepts wolfCrypt cryptographic
     * operations and offloads them to the HSM, is registered for WH_DEV_ID by
     * wh_Client_Init() above, so no explicit registration is needed here.
     */
    printf("  Using wolfHSM crypto callback (devId=0x%08X)\n", WH_DEV_ID);
