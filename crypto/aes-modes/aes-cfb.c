 ...
 * - One-shot encryption using wc_AesCfbEncrypt()
 * - Streaming decryption using wc_AesCfbDecrypt() in chunks
 * Note: CFB mode inherently supports streaming (no padding required)
 */
                        ...
                        const byte* key, word32 keySz)
{
    ...
    /* One-shot encrypt */
    ret = wc_AesCfbEncrypt(&aes, ciphertext + AES_BLOCK_SIZE, plaintext,
                           plaintextSz);
    wc_AesFree(&aes);
    ...
    printf("AES-CFB encryption complete (one-shot)\n");
    return ret;
}
                        ...
                        const byte* key, word32 keySz)
{
    ...
    printf("AES-CFB streaming decryption:\n");
    offset = 0;
    while (offset < plaintextSz) {
        ...
        ret = wc_AesCfbDecrypt(&aes, plaintext + offset,
    ...
    printf("AES-CFB decryption complete (streaming)\n");
    return ret;
}
...
int main(int argc, char** argv)
{
    (void)argc;
    (void)argv;
    printf("AES-CFB not compiled in. Enable with WOLFSSL_AES_CFB\n");
    return 0;
}
