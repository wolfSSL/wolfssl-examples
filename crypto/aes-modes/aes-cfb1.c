 ...
 * - One-shot encryption using wc_AesCfb1Encrypt()
 * - Streaming decryption using wc_AesCfb1Decrypt() in chunks
 * Note: CFB1 processes 1 bit at a time, sz parameter is in bits
 */
...
#define CHUNK_BITS      64  /* Process 64 bits (8 bytes) at a time for streaming demo */
                        ...
                        const byte* key, word32 keySz)
{
    ...
    word32 plaintextSz;
    word32 plaintextBits;
    int    ret;

    ret = read_file(inFile, &plaintext, &plaintextSz);
    if (ret != 0) return ret;

    plaintextBits = plaintextSz * 8;
    ...
    /* One-shot encrypt (sz is in bits for CFB1) */
    ret = wc_AesCfb1Encrypt(&aes, ciphertext + AES_BLOCK_SIZE, plaintext,
                            plaintextBits);
    wc_AesFree(&aes);
    ...
    printf("AES-CFB1 encryption complete (one-shot)\n");
    return ret;
}

/* Streaming decryption - process in chunks of bits */
                        ...
                        const byte* key, word32 keySz)
{
    ...
    /* Streaming decrypt - process in chunks of bits */
    printf("AES-CFB1 streaming decryption:\n");
    offsetBits = 0;
    while (offsetBits < totalBits) {
        chunkBits = totalBits - offsetBits;
        if (chunkBits > CHUNK_BITS) {
            chunkBits = CHUNK_BITS;
        }

        offsetBytes = offsetBits / 8;

        ret = wc_AesCfb1Decrypt(&aes, plaintext + offsetBytes,
                                ciphertext + AES_BLOCK_SIZE + offsetBytes,
                                chunkBits);
        if (ret != 0) {
            wc_AesFree(&aes);
            free(ciphertext);
        ...
        printf("  Decrypted chunk: offset=%u bits, size=%u bits\n",
               offsetBits, chunkBits);
        offsetBits += chunkBits;
    }
    ...
    printf("AES-CFB1 decryption complete (streaming)\n");
    return ret;
}
...
int main(int argc, char** argv)
{
    (void)argc;
    (void)argv;
    printf("AES-CFB1 not compiled in. Enable with WOLFSSL_AES_CFB\n");
    return 0;
}
