 ...
 * - One-shot encryption using wc_AesCfb8Encrypt()
 * - Streaming decryption using wc_AesCfb8Decrypt() in chunks
 * Note: CFB8 processes 8 bits (1 byte) at a time
 */
...
#define CHUNK_SIZE      8  /* Process 8 bytes at a time for streaming demo */
                        ...
                        const byte* key, word32 keySz)
{
    ...
    /* One-shot encrypt */
    ret = wc_AesCfb8Encrypt(&aes, ciphertext + AES_BLOCK_SIZE, plaintext,
                            plaintextSz);
    wc_AesFree(&aes);
    ...
    printf("AES-CFB8 encryption complete (one-shot)\n");
    return ret;
}
                        ...
                        const byte* key, word32 keySz)
{
    ...
    printf("AES-CFB8 streaming decryption:\n");
    offset = 0;
    while (offset < plaintextSz) {
        ...
        ret = wc_AesCfb8Decrypt(&aes, plaintext + offset,
    ...
    printf("AES-CFB8 decryption complete (streaming)\n");
    return ret;
}
...
int main(int argc, char** argv)
{
    (void)argc;
    (void)argv;
    printf("AES-CFB8 not compiled in. Enable with WOLFSSL_AES_CFB\n");
    return 0;
}
