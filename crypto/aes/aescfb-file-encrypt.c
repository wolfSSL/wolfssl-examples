...
int AesDecrypt(Aes* aes, byte* key, int size, FILE* inFile, FILE* outFile)
{
    ...
    /* verify the file is at least large enough to hold the salt and IV
     * before subtracting/allocating based on its length, otherwise a
     * truncated file could underflow the size calculations below */
    if (length < SALT_SIZE + AES_BLOCK_SIZE) {
    ...
    if (ret == 0) {
        if (salt[0] != 0) {
            /* reduces length based on number of padded elements. Validate
             * the padding byte before trusting it as a subtraction amount
             * to avoid an out-of-bounds/garbage-length fwrite on corrupted
             * or malicious ciphertext */
            if (length < 1 || output[length - 1] < 1 ||
                    output[length - 1] > length ||
                    output[length - 1] > AES_BLOCK_SIZE) {
...
{
    XPRINTF("\n~~~~~~~~~~~~~~~~~~~~|Help|~~~~~~~~~~~~~~~~~~~~~\n\n");
    XPRINTF("Usage: ./aes-file-encrypt <-option> <KeySize> <-i file.in> "
        "<-o file.out>\n\n");
    XPRINTF("Options\n");
...
int main(int argc, char** argv)
{
    ...
    if (inCheck == 0 || outCheck == 0) {
        ...
        if (key == NULL) {
            ...
            if (ret == 0) {
                if (choice == 'e')
                    ret = AesEncrypt(&aes, key, size, inFile, outFile);
                else if (choice == 'd')
                    ret = AesDecrypt(&aes, key, size, inFile, outFile);
            }
            else {
                wc_ForceZero(key, size);
                XFREE(key, NULL, DYNAMIC_TYPE_TMP_BUFFER);
