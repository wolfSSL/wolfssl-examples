...
int AesDecrypt(Aes* aes, byte* key, int size, FILE* inFile, FILE* outFile)
{
    ...
    /* input file must be at least large enough to hold the salt and one
     * AES block, otherwise the length arithmetic below would underflow */
    ...
    if (ret == 0) {
        if (salt[0] != 0) {
            /* validate the padding byte before trusting it, to avoid an
             * out-of-bounds / garbage-length fwrite on corrupted or
